...
Is this argument correct? How can we be sure? In natural language we can construct reasons why this argument must be correct, although it is easy for these reasons to be restatements of the argument. In formal logic, we have mathematical tools to help us. There are several ways in which we can prove that this argument is correct, or more specifically that it is valid as we have no way to verify the truth or otherwise of the premises of the argument.We will first consider how to use truth tables to prove the correctness of an argument. Then, we will introduce a technique called *natural deduction* that more closely resembles mathematics to help us.

## Proof by Truth Table

Let us consider how to "prove" our simple argument using a truth table:
...
<<<<<<< HEAD
This is obvious, but can we prove it beyond any residual doubt. One very obvious way to do this is to construct the truth table:
=======
...
<<<<<<< HEAD
If we take the argument to be *sound*, ie the premises are True, then we need to ask two questions to prove the argument:

- In which row of this table are all of the premises True?
- Is the conclusion also True on that row?

If the Conclusion is True when all of the premises are True, then the argument is Valid.

Looking at the table, we see that in the fourth row of the table, both of the premises and the conclusion are True and therefore the argument is correct.

Consider now the argument

> $R\implies U$, $R$, $\therefore \lnot U$

The truth table for this is

| $R$ | $U$ | $R\implies U$ | $\lnot U$ |
|:---:|:---:|:-------------:|:---------:|
|  F  |  F  |  T            |  T        |
|  F  |  T  |  T            |  F        | 
|  T  |  F  |  F            |  T        |
...
<<<<<<< HEAD
We can prove this argument by applying some "rules". Here we will use the following rules that can be rigourously proven by constructing the truth tables (try it):

- Given premises $P\implies Q$ and $P$, then $Q$ must be True. This rule is known as Modus Ponens (latin: affirming the consequent).
- Given premises $P\implies Q$ and $\lnot Q$, then $\lnot P$ must be True. This rule is known as Modus Tollens (latin: denying the antecedent).
- Given premise $\lnot\lnot P$ then $P$ must be True (double negation elimination).
- Given premise $P\land Q$ then both $P$ and $Q$ must be True (and-elimination).

Given these rules, we can apply them to prove the argument as follows:
=======
...
<<<<<<< HEAD
Let us explain some features of the proof
...
- The *premises* on which a proposition depends should be listed in the final column, regardless of whether they have been used directly in the application of a rule. For examples, line 5 does not use premises 1 and 2 directly, but they were used to derive premise 4, which does use them directly.

In sequent form, we write this as
=======
...
<<<<<<< HEAD
This proof of $R$ by **natural deduction** uses only the premises of the argument and some established valid rules of inference. Examples of common inference rules include
=======
...
<<<<<<< HEAD
Each of these inference rules can be semantically proven via a truth table and syntactically applied via symbolic manipulation during a proof.

Let's do another, more complex example. This time, we will consider an *abstract* problem so we can focus on the symbol manipulations.
=======
...
<<<<<<< HEAD
> 1. By showing the the *negation* of the conclusion is true. For example, if the argument concludes $Q$ but you are able to show that $\lnot Q$ is True, then the argument must be invalid.
> 2. By deriving a *contradiction*. If an argument concludes $Q$ and you are able to prove that both $Q$ and $\lnot Q$ are True, then the argument must be invalid.
=======
